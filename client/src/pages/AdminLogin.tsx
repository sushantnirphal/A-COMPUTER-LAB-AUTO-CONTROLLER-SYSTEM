...
function AdminLogin() {
  ...
  async function handleSubmit(event: { preventDefault: () => void }) {
    ...
    const req = await fetch("http://localhost:7890/admin/login", {
      method: "post",
      headers: {
  ...
  useEffect(() => {
    if (location.pathname === "/admin/login" && student?._id) {
      navigate("/home")
      ...
      >
        ...
        >
          <form onSubmit={handleSubmit} className="w-full px-12">
            <h5 className="text-3xl font-bold text-purple_pri-500 py-8">Admin login</h5>
            <div className="mb-4">
